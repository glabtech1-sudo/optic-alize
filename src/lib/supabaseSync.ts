...
export function isBusinessKey(key: string): boolean {
  ...
  // Theme and language must be stored in localStorage.
  // We also keep session auth metadata (tokens/auth state) to maintain browser session persistence.
  const excludedKeys = [
    'optic_app_language',
    'optic_theme_accent',
    'optic_access_token',
...
export async function syncCollectionToSupabase(key: string, data: any): Promise<boolean> {
  ...
  return SupabaseService.syncCollection(key, data, boutiqueName);
}
...
export async function loadCollectionFromSupabase(key: string): Promise<any | null> {
  const boutiqueName = safeLocalStorage.getItem('optic_boutique_name') || 'Global';
  return SupabaseService.loadCollection(key, boutiqueName);
}
