...
async function apiPost<T>(url: string, body: any, fallbackKey: string): Promise<T | null> {
  try {
    ...
    if (res.ok) {
      ...
      console.error('[API] POST failed:', errorData.error || res.statusText);
    }
  } catch (e) {
...
export async function loginUser(email: string, password: string): Promise<AuthResponse> {
  try {
    ...
    return { error: err.message || 'Erreur réseau de connexion.' };
  }
}
...
export async function verifyMFA(sessionId: string, code: string): Promise<AuthResponse> {
  try {
    ...
    return { error: err.message || 'Erreur de vérification MFA.' };
  }
}
